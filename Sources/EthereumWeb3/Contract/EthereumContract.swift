...
public protocol StaticContract: EthereumContract {
    init(address: Address?, eth: Web3.Eth)
}
...
public class DynamicContract: EthereumContract {
    ...
    public init(abi: [ABIObject], address: Address?, eth: Web3.Eth) {
    ...
    public func deploy(byteCode: EthData, parameters: ABIEncodable...) -> SolidityConstructorInvocation? {
    ...
    public func deploy(byteCode: EthData) -> SolidityConstructorInvocation? {
...
extension EthereumContract {
    ...
    public func call(_ call: Call, outputs: [SolidityFunctionParameter], block: QuantityTag = .latest, completion: @escaping (Swift.Result<[String: Any], Error>) -> Void) {
        eth.call(call: call, block: block) { response in
            completion(response.tryMap { data in
                try ABI.decodeParameters(outputs, from: data.hex)
            })
        }
    }
    ...
    public func send(_ transaction: Transaction, completion: @escaping (Swift.Result<EthData, Error>) -> Void) {
        eth.sendTransaction(transaction: transaction) { response in
            completion(response.mapError{$0})
        }
    }
    ...
    public func estimateGas(_ call: Call, completion: @escaping (Swift.Result<Quantity, Error>) -> Void) {

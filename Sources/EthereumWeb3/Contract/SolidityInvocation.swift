...
public protocol SolidityInvocation {
    ...
    func call(block: QuantityTag, completion: @escaping (Swift.Result<[String: Any], Error>) -> Void)
    
    /// Write data to the blockchain. Only available for non-constant functions.
    func send(nonce: Quantity?, from: Address, value: Quantity?, gas: Quantity?, gasPrice: Quantity?, completion: @escaping (Swift.Result<EthData, Error>) -> Void)
    
    /// Estimate how much gas is needed to execute this transaction.
    func estimateGas(from: Address?, gas: Quantity?, value: Quantity?, completion: @escaping (Swift.Result<Quantity, Error>) -> Void)
    
    /// Encodes the ABI for this invocation
    func encodeABI() -> EthData?
...
public struct SolidityReadInvocation: SolidityInvocation {
    ...
    public func call(block: QuantityTag = .latest, completion: @escaping (Swift.Result<[String: Any], Error>) -> Void) {
        guard handler.address != nil else {
            completion(.failure(InvocationError.contractNotDeployed))
            return
        }
        guard let call = createCall() else {
            completion(.failure(InvocationError.encodingError))
            return
        }
    ...
    public func send(nonce: Quantity? = nil, from: Address, value: Quantity?, gas: Quantity?, gasPrice: Quantity?, completion: @escaping (Swift.Result<EthData, Error>) -> Void) {
        completion(.failure(InvocationError.invalidInvocation))
    }
...
public extension SolidityInvocation {
    ...
    func call(completion: @escaping (Swift.Result<[String: Any], Error>) -> Void) {
        self.call(block: .latest, completion: completion)
    }
    
    func estimateGas(from: Address? = nil, gas: Quantity? = nil, value: Quantity? = nil, completion: @escaping (Swift.Result<Quantity, Error>) -> Void) {
    ...
    func encodeABI() -> EthData? {
        if let hexString = try? ABI.encodeFunctionCall(self) {
            return try? EthData(ethereumValue: hexString)
        }
        return nil
    }
}
...
public struct SolidityConstructorInvocation {
    public let byteCode: EthData
    public let parameters: [SolidityWrappedValue]
    public let payable: Bool
    public let handler: SolidityFunctionHandler
    
    public init(byteCode: EthData, parameters: [SolidityWrappedValue], payable: Bool, handler: SolidityFunctionHandler) {
    ...
    public func encodeABI() -> EthData? {
        ...
        return try? EthData(ethereumValue: dataString)
    }
}
